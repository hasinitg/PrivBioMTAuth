...
\begin{enumerate}
 ...
 \item Get two apps communicate and return result.
...
under the res/layout folder, strings.xml file.
 ...
 \item Intent: create Intent : newIntent(), add name-value pairs to it: intent.putExtra(name, value), start activity passing the intent: 
startActivity(intent), receive data sent thru an intent: getIntent(), getStringExtra(name). 
 \item Accessing View elements of an activity through findViewById(R.id.<id\_name>) method.
\end{enumerate}
