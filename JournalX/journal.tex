...
When I merged the branch, it didn't work. Then when I built and deployed the client app again, it worked. Strange.\\
...
logs in, an encrypted token is sent from their phone to USAA that is matched against the ID of the device registered at enrollment. So for a 
fraudster 
...
Secure Enclave: The A7 also includes an area called the "Secure Enclave" that stores and protects the data from the Touch ID fingerprint sensor on 
the 
