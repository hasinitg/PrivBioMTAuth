...
How to measure memory: In Device Monitor, select the device -> select the process -> select `update heap' -> cause GC.\\
Network: Can measure Tx and Rx bytes and packets. There is a way to tag the sockets used for each operation and get network stats for each of them 
as per the aforementioned tutorial. However, it is only possible with android's default http client - URL connection.\\
CPU:
...
4. Inter-app instrumentation\\
4. benchmark, benchmark, benchmark\\


\textbf{MyNexts in end:}\\
